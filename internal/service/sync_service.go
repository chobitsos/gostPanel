...
func (s *RuleSyncService) syncRuleStatus(r model.GostRule, serviceStates map[string]string) {
	...
	if len(serviceIDs) > 0 {
		...
		if r.Status != newStatus {
			...
			if r.Status == model.RuleStatusRunning && newStatus == model.RuleStatusStopped {
				_, _ = s.ruleRepo.MarkStoppedForAutoResumeIfRunning(r.ID)
			} else {
...
func (s *RuleSyncService) syncTunnelStatus(t model.GostTunnel, chainStates map[string]bool) {
	...
	if t.Status != newStatus {
		...
		if t.Status == model.TunnelStatusRunning && newStatus == model.TunnelStatusStopped {
			_, _ = s.tunnelRepo.MarkStoppedForAutoResumeIfRunning(t.ID)
		} else {
